(function (global) {
  ...
  function expand(node, model, state) {
  ...
  async function search(model, rootState, options = {}) {
    ...
    for (let sim = 0; sim < simulations; sim++) {
      ...
      while (true) {
        ...
        if (node.edges == null) {
          leafValue = expand(node, model, state);
          break;
        }
        ...
        if (best.child == null) best.child = newNode();
        node = best.child;
      }
      let value = leafValue;
      ...
      if ((sim + 1) % yieldEvery === 0) await new Promise((resolve) => setTimeout(resolve, 0));
    }
